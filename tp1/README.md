...
Nota sobre filtros: Presionar / o u, escribir el texto y presionar Enter para aplicar. Para limpiar, presionar / o u nuevamente y luego Esc.
...
Resumen	2s	Datos básicos, bajo costo de lectura
Memoria	3s	Lectura de /proc/<pid>/maps es costosa
FDs	5s	Los FDs cambian poco frecuentemente
Threads	2s	Los threads pueden cambiar rápido
Señales	10s	Las máscaras de señales son muy estables
Scheduling	10s	La política de scheduling rara vez cambia
Sistema	2s	CPU y memoria global cambian constantemente
...
docker compose up --build sin attach: con este comando las teclas no responden porque Docker Compose no conecta stdin directamente al contenedor. Se recomienda usar docker compose run --rm monitor.
...
El dump JSON queda dentro del contenedor: cuando se envía SIGUSR1, el archivo dump_<timestamp>.json se crea en /app/ dentro del contenedor. Para accederlo: docker exec <contenedor> cat /app/dump_*.json.

Un solo FD por proceso en la vista FDs: el snapshot almacena todos los FDs pero por espacio en pantalla se muestra solo el primero.
...
docker ps                                                    # anotar nombre del contenedor
...
El layout muestra siempre una lista de procesos arriba (con PID, PPID, Usuario, Estado, CPU%, RSS, Threads, Comando) y un panel de detalle abajo que cambia según la vista activa. El título del panel muestra la vista activa, el intervalo de refresco, el orden y los filtros activos.
